...
class TestIntegration(object):
    ...
    def test_simple_linear_regression_run(self):
        inputs = ["--input-data-file", "data/input/ex1data1.txt", "--number-features", '1', "--number-targets", '1', "--hypothesis-name", 'simple_linear_regression', "--cost-function-name", 'squared_error_loss', "--algorithm-name", "batch_gradient_descent", "--learning-rate", "0.001", "--tolerance", '.000000001']
        learning_model_obj = runmain.main(inputs)
        parameters = learning_model_obj.get_parameter_point_estimates()
        assert_almost_equals(parameters[0,0], SIMPLE_LINEAR_REGRESSION_INTERCEPT, places=4)
        assert_almost_equals(parameters[1,0], SIMPLE_LINEAR_REGRESSION_SLOPE, places=4)

    def test_multiple_linear_regression_squared_error_loss_batch_gradient_descent_run(self):
        inputs = ["--input-data-file", "data/input/sample_data.csv", "--number-features", '2', "--number-targets", '1', "--hypothesis-name", 'multiple_linear_regression', "--cost-function-name", 'squared_error_loss', "--algorithm-name", "batch_gradient_descent", "--learning-rate", "0.0001", "--tolerance", '.0000000001']
        learning_model_obj = runmain.main(inputs)
        parameters = learning_model_obj.get_parameter_point_estimates()
    ...
    def test_percetron_batch_run(self):
        ...
        assert_almost_equals(parameters[1,0], PERCEPTRON_BATCH_FEATURE_1, places=4)
        assert_almost_equals(parameters[2,0], PERCEPTRON_BATCH_FEATURE_2, places=4)

    def test_perceptron_online_run(self):
        inputs = ["--input-data-file", "data/input/sample_perceptron.csv", "--number-features", '2', "--number-targets", '1', "--hypothesis-name", "perceptron", "--cost-function-name", "perceptron_online_cost", "--algorithm-name", "stochastic_gradient_descent", "--learning-rate", '1.']
        learning_model_obj = runmain.main(inputs)
        parameters = learning_model_obj.get_parameter_point_estimates()
        assert_almost_equals(parameters[0,0], PERCEPTRON_ONLINE_BIAS, places=4)
        assert_almost_equals(parameters[1,0], PERCEPTRON_ONLINE_FEATURE_1, places=4)
