...
class Regularizer(object):
    ...
    def ridge_function(cls, regularization_matrix, parameter_vector):
        return regularization_matrix.dot(parameter_vector).T.dot(parameter_vector)
    ...
    @classmethod
    def regularizer_cost_function(cls, name, regularization_matrix, parameter_vector):
        if name == "ridge":
    ...
    @classmethod
    def regularizer_cost_function_derivative(cls, name, regularization_matrix, parameter_vector):
        if name == "ridge":
            return cls.ridge_derivative(regularization_matrix, parameter_vector)
        else:
            return 0

    @classmethod
    def regularizer_cost_function_second_derivative(cls, name, regularization_matrix, parameter_vector):
        if name == "ridge":
