...
The log-likelihood is:
    l_nobs(Y|X ; THETA) = -nobs/2 * ln(sigma^2) - nobs/2 * ln(2PI) -
...
class MaximumLikelihoodNormalDistribution(CostFunction):
    def __init__(self, hypothesis, targets):
        """
        """
        super(MaximumLikelihoodNormalDistribution, self).__init__(
            hypothesis, targets)
    ...
    def maximum_likelihood_fit_function(self):
        # TODO: put the error variance in matrix form such that it is a
        # diagonal matrix, in future multiple errors uncorrelated.
        conditional_mean = self.hypothesis.conditional_mean()
        ml_fit_function = (log(self.error_variance.value) -
                           log(var(self.targets)) +
                           1./self.error_variance.value*var(self.targets) -
                           1. + ((mean(self.targets) -
                                  conditional_mean)).T.dot(
                                      1./self.error_variance.value).dot((
                                          mean(self.targets) -
                                          conditional_mean)))
    ...
    def cost_function_derivative(self):
        ...
        conditional_mean_derivative = (
            self.hypothesis.features.T.dot(
                self.targets - conditional_mean))/(self.error_variance.value)
        error_variance_derivative = -1.*self.nobs/(
            2.*self.error_variance.value) + ((
                self.targets - conditional_mean).T.dot(
                    self.targets - conditional_mean))/(
                        2.*self.error_variance.value**2)
        return vstack((-1.*conditional_mean_derivative,
                       -1.*error_variance_derivative))

    def cost_function_second_derivative(self):
        # TODO: put the error variance in matrix form such that it is a
        # diagonal matrix, in future multiple errors uncorrelated.
        conditional_mean = self.hypothesis.conditional_mean()
        conditional_mean_covariance = (
            -1.*self.hypothesis.features.T.dot(
                self.hypothesis.features))/self.error_variance.value
        # conditional_mean_and_error_variance_covariance =
        # (-1.*self.hypothesis.features.T.dot(self.targets -
        ...
        error_variance_covariance = (
            self.nobs)/(2.*self.error_variance.value**2) - ((
                self.targets - conditional_mean).T.dot(
                    self.targets - conditional_mean))/(
                        self.error_variance.value**3)
        tmp_top = hstack((
            conditional_mean_covariance,
            conditional_mean_and_error_variance_covariance))
        tmp_bottom = hstack((
            conditional_mean_and_error_variance_covariance.T,
            error_variance_covariance))
    ...
    def cost_function(self):
        ...
        """
        conditional_mean = self.hypothesis.conditional_mean()
        log_likelihood = (-1.*self.nobs)/2. * log(
            self.error_variance.value) - self.nobs/2. * log(2.*pi) - (1.)/(
                2.*self.error_variance.value) * (
                    self.targets - conditional_mean).T.dot(
                        self.targets - conditional_mean)
