...
class ModelSetup(object):
    def __init__(self, hypothesis_type, algorithm_type,
    ...
    def check_dependencies(self):
        ...
        """
        if self.cost_function_type is not None:
    ...
    def model_setup(self, features, targets=None, regularizer_name=None,
                    regularization_weight=None, k=None, learning_rate=None,
        ...
        """
        optional_hyp_fnx_arguments = [{"k": k}]
        hyp_kwargs = {key: val
                      for optional_argument in optional_hyp_fnx_arguments
                      for key, val in optional_argument.items()
                      if val is not None}
        hypothesis_obj = self.set_hypothesis(features, **hyp_kwargs)
        optional_cost_fnx_arguments = [
            {"targets": targets},
            {"regularizer_name": regularizer_name},
            {"regularization_weight": regularization_weight}]
        if self.cost_function_type is not None:
            cost_fnx_kwargs = {key: val
                               for optional_argument in
                               optional_cost_fnx_arguments
                               for key, val in optional_argument.items()
                               if val is not None}
            cost_function_obj = self.set_cost_function(hypothesis_obj,
                                                       **cost_fnx_kwargs)
        optional_alg_arguments = [
        ...
        algorithm_kwargs = {key: val
                            for optional_argument in optional_alg_arguments
                            for key, val in optional_argument.items()
                            if val is not None}
        if self.cost_function_type is not None:
            algorithm_obj = self.set_algorithm(cost_function_obj,
                                               **algorithm_kwargs)
        else:
            algorithm_obj = self.set_algorithm(hypothesis_obj,
                                               **algorithm_kwargs)
